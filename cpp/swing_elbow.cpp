...
class G1Example {
 private:
  ...
  double duration_;    // [3 s] transition to zero
  double right_elbow_cmd_;   // ramped command (rad), follows goal at elbow_ramp_rate_
  double elbow_ramp_rate_;  // rad/s — lower = angle is followed more slowly
  int counter_;
  Mode mode_pr_;
  ...
  std::atomic<double> right_elbow_target_rad_{0.0};  // keyboard-controlled target in radians
 ...
 public:
  ...
  G1Example(std::string networkInterface,
      ...
      : time_(0.0),
        ...
        elbow_ramp_rate_(3),  // rad/s (~28 deg/s); increase for faster tracking
        counter_(0),
        mode_pr_(Mode::PR),
        mode_machine_(0),
  ...
  void Control() {
    ...
    if (ms) {
      ...
      if (time_ < duration_) {
        // [Stage 1]: transition to zero posture
        for (int i = 0; i < G1_NUM_MOTOR; ++i) {
          double ratio = std::clamp(time_ / duration_, 0.0, 1.0);
          motor_command_tmp.q_target.at(i) = (1.0 - ratio) * ms->q.at(i);
        }
      } else {
        // [Stage 2]: keyboard-controlled right elbow (start 0 rad, u +0.1 rad, d -0.1 rad)
        ...
        motor_command_tmp.q_target.at(RightElbow) = right_elbow_cmd_;
      }
